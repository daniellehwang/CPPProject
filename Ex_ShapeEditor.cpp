...
int main()
{
	...
	//Rect v1[30]; //Rect만 관리한다
	//Circle v2[30]; //Circle만 관리한다

	//vector< Rect > v1;	//Rect만 관리한다
	//vector< Circle > v2;	//Circle만 관리한다


	vector< Shape* > v; //부모의 포인터로는 자식을 가르킬 수 있다(x error)
						//자식을 포인터로는 부모를 가르킬 수 없다(error)
	...
	while (1)
	{
		...
		case 9://All Draw
			for (auto& p : v)
			{
				p->Draw();//정적 바인딩(기본) ->동적(실행) 바인딩(virtual)
